...
if ( ! class_exists( 'XPSC_Woo_Sale_Countdown' ) ) :
	...
	define( 'XPSC_DOMAIN', 'emerce-core' );
	...
	class XPSC_Woo_Sale_Countdown {
		...
		public function sale_countdown() {
			...
			?>
				<h5 class="emerce-sale-countdown-title"><?php echo esc_html( cs_get_option( 'countdown_ttl' ) ); ?></h5>
			<?php
...
<div class="emerce-timer-for-countdown emerce-timer-<?php echo cs_get_option('count_style');?> <?php echo cs_get_option('sdw_enable');?>">
		<div id="emercetimer"><?php echo $sale_to_date; ?></div>
		</div>
		...
		private function product_stock_progress_bar() {
			...
			if ( ( 'no' === cs_get_option('stockbar_progress') ) || ( ! $product->get_manage_stock() ) || ( 0 == $product->get_stock_quantity() ) ) {
				return '';
			}
