<?php
if(!class_exists("EmerceBase")) {
	class EmerceBase {
    	public $key = "5B7CB20CFD6E1FCB";
    	private $product_id = "3";
    	private $product_base = "emerce";
    	private $server_host = "https://license.teconce.com/wp-json/teconce_license/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_emerce_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=emerce_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
