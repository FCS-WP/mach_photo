...
if (!function_exists('emerce_wc_get_gallery_image_html')) {
    // Copied and modified from woocommerce plugin and wc_get_gallery_image_html helper function.
    function emerce_wc_get_gallery_image_html($attachment_id, $main_image = false, $size = 'woocommerce_single')
    {
...
function pivoo_woo_inventory_functions()
{
    global $post, $product;
    if ($product->get_stock_quantity()) { // if manage stock is enabled
        if (number_format($product->get_stock_quantity(), 0, '', '') < 10) { // if stock is low
            echo '<span class="pivoo-remaining-stock red-notice">Only ' . number_format($product->get_stock_quantity(), 0, '', '') . ' left in stock</span>';
        } else {
            echo '<span class="pivoo-remaining-stock green-notice">' . number_format($product->get_stock_quantity(), 0, '', '') . ' left in stock</span>';
        }
    }
}

add_action('pivoo_woo_inventory_hook', 'pivoo_woo_inventory_functions');
...
{
    $defaults['delimiter'] = '<i class="zil zi-chevron-right"></i> ';
    return $defaults;
}
...
if (class_exists('YITH_WCWL')) {
     ...
     */
    function emerce_product_wishlist_button()
    {
        ?>
        <div class="emerce-wishlist-sp">
    ...
    add_action('woocommerce_after_add_to_cart_button', 'emerce_product_wishlist_button', 10, 0);
