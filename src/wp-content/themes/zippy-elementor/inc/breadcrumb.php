 ...
 * @since    1.0
 */
if (!function_exists('emerce_breadcrumbs')) {
    function emerce_breadcrumbs()
    {
        global $wp_query, $post, $paged;
        $space = ' ';
        $on_front = get_option('show_on_front');
        $blog_page = get_option('page_for_posts');
        $separator = $space . '<span class="bredcrumb-separator active"><i class="ri-arrow-right-s-line"></i></span>' . $space;
        ...
        if ($on_front == "page" && is_home()) {
            $blog_title = isset($blog_page) ? get_the_title($blog_page) : __('Our Blog', 'emerce');
            $out .= sprintf($link, home_url(), __('Home', 'emerce')) . $separator . sprintf($current, $blog_title);
        } else {
            $out .= sprintf($link, home_url(), __('Home', 'emerce'));
        }
        if (is_singular()) {
            ...
            if (is_post_type_archive()) {
                ...
                $out .= $separator . __('<span class="active">Category ></span> ', 'emerce') . sprintf($current, $wp_query->queried_object->name);
            } else if (is_tag()) {
                $out .= $separator . __('Tag: ', 'emerce') . sprintf($current, $wp_query->queried_object->name);
            } else if (is_date()) {
                $out .= $separator;
                if (is_day()) {
                    global $wp_locale;
                ...
                $out .= $separator . sprintf($current, __('Error 404', 'emerce'));
            } elseif (is_search()) {
                if (have_posts()) {
        ...
        $out .= '</div>';
        echo apply_filters('emerce_breadcrumbs_out', $out);
    }
}
// Hooks into the Digital Store theme
add_action('emerce_before_template_header', 'emerce_breadcrumbs');