...
class Merlin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		$config = wp_parse_args(
			$config, array(
				...
				'merlin_url'           => 'emerce-wizard',
				'parent_slug'          => 'emerce-admin-menu',
	...
	function required_classes() {
		...
		require_once trailingslashit($this->base_path) . $this->directory . '/includes/class-merlin-csf-importer.php';
	...
	public function admin_page() {
		...
		wp_add_inline_style( 'merlin', '#qm-icon-container,#query-monitor-main{display:none}' ); // Hide Query Monitor plugin.
		 ...
		 wp_enqueue_style( 'merlin-dashboard-style',EMERCE_URL . '/assets/css/merlin-dashboard.min.css' );
		 
		 wp_enqueue_style( 'jquery-confirm-css',EMERCE_URL . '/assets/css/jquery-confirm.min.css' );
		...
		wp_enqueue_script('jquery-confirm',EMERCE_URL . '/assets/js/jquery-confirm.min.js', array('jquery'), '5.0', true);
		
		wp_enqueue_script('jquery-custom-select',EMERCE_URL . '/assets/js/jquery.customselect.min.js', array('jquery'), '5.0', true);
...
/**
	...
	protected function license() {
		...
		<?php
		$this->logger->debug( __( 'The license activation step has been displayed', 'emerce' ) );
	}
	...
	protected function content() {
		...
		<div class="merlin__content--transition merlin__content--transition-full-wdth">
		    
		    <?php if (class_exists( 'Emerce_Core' ) &&  class_exists( 'woocommerce' )) { ?>
			...
			<div class="demo-import-required-options">
			    
			    <h5 class="before_import_notice_merlin">Before importing, please ensure that everything is green.</h5>
			    
			    <table class="system-status-table-merlin">
				...
				<div class="merlin__select-control-wrapper">
					...
					<select class="merlin__select-control js-merlin-demo-import-select">
						...
						<?php foreach ( $this->import_files as $index => $import_file ) : ?>
							...
							<option value="<?php echo esc_attr( $index ); ?>" data-img-src="<?php echo esc_html( $import_file['import_preview_image_url'] ); ?>">
							    
							    
							    <?php echo esc_html( $import_file['import_file_name'] ); ?></option>
			...
			<a id="merlin__drawer-trigger" class="merlin__button merlin__button--knockout"><span><?php echo esc_html( $action ); ?></span><span class="chevron"></span></a>
			<?php } else { ?>
			<h4 class="dh-plugin-required">Please Install and Activate Emerce Core & WooCommerce Plugin to continue import demo.</h4>
			<?php } ?>
	...
	protected function ready() {
		...
		<footer class="merlin__content__footer merlin__content__footer--fullwidth <?php echo esc_attr( $links_class ); ?>">
			...
			<a href="<?php echo esc_url( home_url( '/' ) ); ?>" target="_blank" class="merlin__button merlin__button--blue merlin__button--fullwidth merlin__button--popin"><?php echo esc_html( $big_btn ); ?></a>
	...
	public function generate_child_functions_php( $slug ) {
		...
		$output = "
			 ...
			 * This child theme was generated by Auto Installer.
			 *
	...
	public function generate_child_style_css( $slug, $parent, $author, $version ) {
		...
		$output = "
			...
			* Description: This is a child theme of {$parent}, generated by Auto Installer.
			* Author: {$author}
			* Template: {$slug}
			* Version: {$version}
			*/\n
    ...
    public function get_import_data_info($selected_import_index = 0)
    {
        ...
        if (
            !empty($this->import_files[$selected_import_index]['import_csf']) ||
            !empty($this->import_files[$selected_import_index]['local_import_csf'])
        ) {
            $import_data['csf'] = true;
        }
...
/**
    ...
    protected function get_import_data($selected_import_index = 0)
    {
        ...
        if (!empty($import_files['csf'])) {
            $content['csf'] = array(
                'title' => esc_html__('Theme Options', '@@textdomain'),
                'description' => esc_html__('CSF Framework Options.', '@@textdomain'),
                'pending' => esc_html__('Pending', '@@textdomain'),
                'installing' => esc_html__('Installing', '@@textdomain'),
                'success' => esc_html__('Success', '@@textdomain'),
                'install_callback' => array('Merlin_Csf_Importer', 'import'),
                'checked' => $this->is_possible_upgrade() ? 0 : 1,
                'data' => $import_files['csf'],
            );
        }
  ...
  /**
    ...
    public function get_import_files_paths($selected_import_index)
    {
        ...
        // Get CSF import file as well. If defined!
        if (!empty($selected_import_data['import_csf'])) {
            $csf_items = array();

            // Setup filename paths to save the Redux content.
            foreach ($selected_import_data['import_csf'] as $index => $redux_item) {
                $redux_filename = 'csf-' . $index . '-' . $base_file_name . '.json';
            ...
            // Download the Redux import file.
            $import_files['csf'] = $redux_items;
        } elseif (!empty($selected_import_data['local_import_csf'])) {
            $csf_items = array();

            // Setup filename paths to save the Redux content.
            foreach ($selected_import_data['local_import_csf'] as $redux_item) {
                if (file_exists($redux_item['file_path'])) {
                    $csf_items[] = $redux_item;
                }
            }

            // Download the Redux import file.
            $import_files['csf'] = $csf_items;
        }
	...
	function elementor_update_options() {
		...
		update_option( 'elementor_cpt_support', [ 'page', 'post', 'product_template', 'footer_builder', 'edd_archive_builder', 'emerce_block' ] );
	}
